...
class HandPose():
    def __init__(self, input_source = 0, model, nPoints, POSE_PAIRS):
        self.input_source = input_source
        self.protofile,self.weightfile = model
        self.nPoints = nPoints
        self.POSE_PAIRS = POSE_PAIRS
        self.threshold=0.2
        self.vid_writer =cv2.VideoWriter(output.avi, cv2.VideoWriter_fourcc(*'MJPG'), 15, (frame.shape[1], frame.shape[0]))
